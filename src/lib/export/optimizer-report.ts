...
export function formatOptimizerReportMarkdown(name: string, job: JobDescription, fit: FitReport): string {
  ...
  const lines: string[] = [
    ...
    `- Responsibilities: ${o.jdMatch.responsibilityMatch}% (${o.jdMatch.responsibilityMatchedCount ?? 0}/${o.jdMatch.responsibilityTotalCount ?? 0} duty lines)`,
    `- ATS keywords: ${o.jdMatch.atsKeywordMatch}%`,
    `- Education/Certifications: ${o.jdMatch.educationCertMatch}%`,
    `- Leadership: ${o.jdMatch.leadershipMatch ?? "—"}%`,
    `- Communication: ${o.jdMatch.communicationMatch ?? "—"}%`,
    `- AI relevance: ${o.jdMatch.aiRelevanceMatch ?? "—"}%`,
    "",
    ...
    "",
    "## 8b. Responsibility evidence map",
    ...(o.responsibilityHighlights?.map((h) => `- **${h.coverage}%** ${h.responsibility}\n  - ${h.evidenceSnippet}`) || ["(no duty lines parsed)"]),
    ...(o.responsibilityGaps?.length ? ["", "**Gaps:**", ...o.responsibilityGaps.map((g) => `- ${g}`)] : []),
    "",
